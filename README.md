...
> ACM 班 2026 级程序设计第一次大作业

~~大作业~~ 扫雷，启动！

## 目录

- [Minesweeper-2026](#minesweeper-2026)
  - [目录](#目录)
  - [简介](#简介)
    - [背景](#背景)
    - [作业目标](#作业目标)
  ...
  - [须知](#须知)
    ...
    - [提交方式](#提交方式)
    - [评分规则](#评分规则)
      - [Advanced评分准则](#advanced评分准则)
        - [写在前面](#写在前面)
        - [关于Baselines](#关于baselines)
        - [关于下发数据](#关于下发数据)
...
初始化完成后，你需要输出地图（格式见下）。随后，你会通过标准输入得到若干行字符串来表示玩家的操作，每一行有三个非负整数 `x y type` 。 `x` ， `y` 是两个在 `[0, n - 1]` 和 `[0, m - 1]` 范围的整数，代表玩家这一步希望操作第 `x` 行第 `y` 列的格子。 `type` 的取值只可能 0 , 1 或 2 ，代表操作的类型： 0 表示访问格子， 1 表示标雷， 2 表示自动探索。
...
- 输出 `n` 行 `m` 列，最后输出换行符
  ...
  - 对于已被访问过的格子，若其为地雷，输出 `X`，若其不是地雷，输出一个 `0` 到 `8` 的数字字符 `k`，表示其地雷计数。例如对于上面的地图，若依次访问坐标为 (2, 0) , (2, 2) 的格子，输出应当为
      ```
      ???
      12?
      01X
...
因此，如你所见，你需要在 ReadMap 函数中读取有限的地图信息，并储存到一些全局变量中，随后根据这些地图信息做出下一步的决策，通过 `Execute` 再次发起决策。具体的代码结构逻辑可以参考 `advanced.cpp` 和 `client.h` 。如果你不明白代码的实现方式，请立即询问助教！！！
...
你也可以在项目根目录运行以下命令，自动编译并检查所有同时提供 `.in` 和 `.out` 的公开测试：

```bash
python3 check.py
```

该脚本只会在系统临时目录中生成编译文件和实际输出，不会修改你的源码或测试数据。只有 `.in` 而没有对应 `.out` 的进阶测试不会被该脚本检查。

#### 进阶任务

对于进阶任务，你的程序主文件是 `advanced.cpp`，其内容无需更改，你只需要阅读并理解该程序是如何调用 `server.h` 与 `client.h` 中的函数的。在完成基础任务之后，你需要实现 `client.h` 中标记 `TODO` 的函数，并通过运行 `advanced.cpp` 来测试你的程序。在 `testcases/advanced` 文件夹中，有 5 个公开数据点供你完成测试。更详细地，以 `adv1` 测试点为例，请将 `adv1.in` 输入你的程序，并等待程序的输出。
...
如果你认为评分机制有不合理之处， advanced 部分时限过短或者一些其他改进建议，可以私信本次大作业的负责助教@RosettaNebula（王文轩）/@serene_analysis（苏成蹊）提出。我们会认真考虑各位的意见并可能作出调整。
...
本次作业改编自2025程序设计的第一次大作业 Minesweeper-2025 。 感谢 22、23、24级负责助教提供的 Idea 和代码框架。
